...
import { HiViewGrid, HiCollection, HiPlusCircle, HiChartBar, HiShoppingCart, HiUser, HiLogout, HiX } from 'react-icons/hi';
import { useAuth } from '../context/AuthContext';

const links = [
  { to: '/', icon: HiViewGrid, label: 'Dashboard' },
  { to: '/portfolios', icon: HiCollection, label: 'Portfolios' },
  { to: '/portfolios/create', icon: HiPlusCircle, label: 'Create Portfolio' },
  { to: '/sales', icon: HiShoppingCart, label: 'Sales' },
        ...
        <div className="flex items-center justify-between p-5 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <span className="text-[10px] text-emerald-400 font-medium uppercase tracking-[0.15em]">Partner</span>
            </div>
          </div>
        ...
        <nav className="px-3 space-y-0.5">
          {links.map(l => (
            <NavLink key={l.to} to={l.to} end={l.to === '/'} onClick={onClose}
              className={({ isActive }) => `flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-[13px] font-medium transition-all duration-300 group ${isActive ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'text-gray-500 hover:text-gray-300 hover:bg-white/[0.03] border border-transparent'}`}>
